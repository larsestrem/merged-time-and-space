...
| 19f | `build-questions.mjs` | `/questions/` — **the questions-first front door**: the seven questions a curious student actually asks (why don't planets fall in, how gravity and motion make an orbit, what changing one part does, seasons, phases, tides, Jupiter's moons), each answered honestly in two paragraphs and ORDERED AS A CHAIN — every card ends by handing the reader the next question, and the last hands them the ask ("the eighth question is yours" → work-with-us). Each answer carries "see it happen" links into the simulator that tests it. Every figure derived: tilt from the daynight solar series, speeds from the sun's GM + the proven element table, Jupiter/Earth from the two GMs the moon systems are checked against, the moon's cycle from the orrery's own span constant. The whole set doubles as FAQPage JSON-LD. Linked from the nav (heads the space block), the sitewide footer's topic line, the home guides line, /about/, /classroom/ and the orbital + day/night pages |
