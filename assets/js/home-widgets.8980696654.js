...
(function(){
  var row=document.getElementById('home-tabs'); if(!row) return;
  var tabs=[].slice.call(row.querySelectorAll('[data-hg-tab]'));
  var blocks=[].slice.call(document.querySelectorAll('[data-hg]'));
  /* the intro line for the current tab. A separate attribute from data-hg
     because these swap one-for-one rather than filtering a set. */
  var ledes=[].slice.call(document.querySelectorAll('[data-hg-lede]'));
  var rows=[].slice.call(document.querySelectorAll('.home-tools-row,.home-mini-row'));
  function show(tab){
    var i;
    for(i=0;i<blocks.length;i++){
      var g=blocks[i].getAttribute('data-hg').split(' ');
      /* data-hg-not lists tabs a block stays OUT of even when they would
         otherwise include it. The classroom cards use it to stay off All:
         they are written to a teacher rather than to whoever turns up, and
         nine of them in the middle of the everything view would bury the
         tools that view exists to show. */
      var not=blocks[i].getAttribute('data-hg-not');
      blocks[i].hidden = (not && not.split(' ').indexOf(tab)>=0)
        || (tab!=='all' && g.indexOf(tab)<0);
    }
    /* a row with nothing left in it is a gap, not a row */
    for(i=0;i<rows.length;i++){
      if(rows[i].hasAttribute('data-hg')) continue;
      var kids=rows[i].children, vis=0, j;
      for(j=0;j<kids.length;j++) if(!kids[j].hidden) vis++;
      rows[i].hidden = vis===0;
    }
    for(i=0;i<ledes.length;i++) ledes[i].hidden = ledes[i].getAttribute('data-hg-lede')!==tab;
    for(i=0;i<tabs.length;i++)
      tabs[i].setAttribute('aria-selected', tabs[i].getAttribute('data-hg-tab')===tab?'true':'false');
  }
  var start='all';
  try{ var q=new URLSearchParams(location.search).get('tab');
       if(q&&document.getElementById('home-tab-'+q)) start=q; }catch(e){}
  for(var i=0;i<tabs.length;i++) tabs[i].hidden=false;
  show(start);
  row.addEventListener('click',function(e){
    var b=e.target.closest('[data-hg-tab]'); if(!b) return;
    var t=b.getAttribute('data-hg-tab');
    show(t);
    try{ history.replaceState(null,'', t==='all'?location.pathname:location.pathname+'?tab='+t); }catch(err){}
...
function dnTwiPath(dec,ss,step){
  ...
  var g=document.getElementById('wk-night');
  if(g){
    var tick=function(){
