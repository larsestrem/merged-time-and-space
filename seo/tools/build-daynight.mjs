...
function locAsk(manual){
  ...
  navigator.geolocation.getCurrentPosition(locFound,function(){
    /* on the automatic attempt this is silent — the button simply appears,
       which is the whole message. Asked for deliberately, it earns a reason. */
    if(locB) locB.hidden=false;
...
if(locB){
  if(!navigator.geolocation){ if(locB.parentNode) locB.parentNode.removeChild(locB); locB=null; }
  else{
    locB.disabled=false;
    locB.addEventListener('click',function(){ locAsk(1); });
  }
}
...
const simCard = `  <div class="card dn-card">
    <div class="dn-figwrap">
      ${MAP_SVG}
      ${/* THE TWO BUTTONS SIT ON THE MAP, along its bottom edge — Now hard left,
           Play hard right. They belong to the picture rather than to a control
           panel under it, and putting them there costs the picture no height at
           all, which on a 2.5:1 map is the dimension worth protecting. */""
      }<div class="dn-overlay">
        <span class="dn-oleft">
          <button type="button" class="chip dn-obtn" id="dn-now" hidden>Now</button>
          ${/* THE LOCATION BUTTON IS THE FALLBACK, NOT THE ROUTE. The page asks
               on load; this only appears if that was refused or timed out, and
               it is removed outright on a device with no geolocation. */""
          }<button type="button" class="chip dn-obtn dn-oicon" id="dn-loc" aria-label="Put my location on the map" title="Put my location on the map" hidden disabled>${PIN}<span class="dn-oicon-t">My location</span></button>
        </span>
